...
const register = async (userData: UserData): Promise<AuthResponse> => {
  try {
    const response = await axios.post<AuthResponse>(`${API_URL}/api/client/register`, userData);
...
const login = async (userData: UserData): Promise<string> => {
  try {
    const response = await axios.post<string>(`${API_URL}/api/client/token`, userData);
